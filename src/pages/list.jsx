...
export default class extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.movies !== this.state.movies) {
      console.log("LIST JE UPDATEOVAN");
      this.setState({ movies: this.props.movies });
    }
  }
  ...
  render() {
    ...
    let moviesTitleRender = null;
    if(this.props.genre) {
      moviesTitleRender = (
        <h1 className="secondary-color weight500 font24">{"Top " + this.props.genre + " Movies"}</h1>
      );
    }else {
      moviesTitleRender = (
    ...
    if (this.state.pickedBest && this.state.movies) {
      ...
      MoviesRender = movieData.reverse().map((movie) => (
        <SingleMovie
          key={movie.id}
          film={movie}
          openInfoPopup={this.props.openInfoPopup}
      ...
      if (this.props.genre) {
        moviesTitleRender = (
          <h1 className="secondary-color weight500 font24">
            {"Lowest " + this.props.genre + " Movies"}
          </h1>
        );
      } else {
        moviesTitleRender = (
          <h1 className="secondary-color weight500 font24">Lowest Rated Movies</h1>
        );
      }
      ...
      if(this.props.genre) {
        moviesTitleRender = (
          <h1 className="secondary-color weight500 font24">
            {"Random " + this.props.genre + " Movies"}
          </h1>
        );
      }else {
        moviesTitleRender = (
